...
class Actuation():
    ...
    def stepForward(self):
        ...
        #     print(f"Postion = {self.position[0]} , {self.position[1]}")
        #     print(self.agent.environment.displaySurface.get_at((int(self.position[0]) , int(self.position[1])+10))) #added 10 as height of robot is 10, it looks just infront of it.
        #     self.timeStepCounter+=1
        #     if(self.timeStepCounter >= 30):
        #         localRects = self.agent.environment.rectList
        #         for r in range(0,len(localRects)):
        #             if (localRects[r].collidepoint(self.position[0], self.position[1])):
        #                 print(f"Rect ID = {r}")
        #                 break
        #         self.timeStepCounter = 0
        self.agent.body.rect.centerx = self.position[0]
